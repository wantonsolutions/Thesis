...
and without programmable network devices). \swordbox demonstrates how caching can be used to remove
...
\swordbox takes the stance that a single programable switch sees all traffic within the rack and
...
similar to swordbox. More generically they could be used for some of the simple functions which are
difficult to implement in remote memory such as an allocator.
...
more complex atomic operations are less present. The algorithms community has designed many
wait-free and lockless data structures such as binary trees, and heaps, and other simple structures
which make use of multi-cas, the ability to CAS multiple addresses simultaneously could open up
disaggregation to many pointer based data structures. Simultaneously CAS and FAA are limited width.
Extensions for larger CAS sizes would enable more efficient data structures such as in the case of
RACE which must limit the size of its key-value pairs due to the lack of space in the 64 bit RDMA
CAS.
