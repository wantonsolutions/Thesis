...
trends have lead to intra rack latencies of 1-2{$\mu$}s and network bandwidths on the same order as memory
...
differs from prior distributed systems due to its scale, and system assumptions. Disaggregation
...
disaggregating a resource is proportional its access latency with respect to the additional latency
...
widely available and its performance and scalability are not well understood. Regardless of
interconnect the principle of access latency remains the same. This dissertation focuses on RDMA although
