...
exploding modern network hardware. For instance \textit{Where should serialization occur?} has no
well established answer, RDMA atomics are only one mechanism available. Given the landscape of
programmable hardware in-network techniques for serialization are possible but hardly
explored~\cite{supernic,clio,mind}. Simultaneously \textit{What data structures should be used?} is
likewise unanswered -- current disaggregated data structures are custom fit for their tasks and
require years of development~\cite{clover,fusee,race,sherman,rolex,ditto}. Understanding the
interplay between data structure and network hardware is the core of this work.
