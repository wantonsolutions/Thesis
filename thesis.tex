...
We have presented two systems, \textsc{Sword} and RCuckoo, that present two different strategies for
improving the performance of disaggregated data structures. \textsc{Sword} uses a programmable
...
In this chapter, we begin by summarizing the contributions of this dissertation and their relationship to our thesis claim. We conclude with a discussion of this work's limitations paired with future work directions in this area of research.
...
efficient disaggregated systems. In the case of \textsc{Sword}, which demonstrated acceleration on
...
Many off-the-shelf ARM-based or FPGA-based SmartNICs could be used to implement \textsc{Sword} as
well as more complex data structures. While SmartNICs lack a global view of a rack, ARM-based
SmartNICs have a global view of the machine they are attached to and could be used to implement
similar caching strategies. SmartNICs typically have much more available memory than programmable
switches and could likely handle much larger data structures than \textsc{Sword}. For example,
inserting into a linked list (because the entire linked list needs to be stored in memory) is a
\textit{difficult} data structure for \textsc{Sword} to handle. This is unlikely to be the same on a
SmartNIC with a few GB of memory. SmartNICs could be used to cache index structures for large
structures like B-Trees and use their limited compute power to steer read and write requests similar
to \textsc{Sword}. More generically, they could be used for simple functions that are difficult to
implement in remote memory, such as an allocator or scheduler that needs to maintain centralized
state.
...
\textsc{Sword}, Clover was a good fit for the system as only the tail pointer of linked lists needed
