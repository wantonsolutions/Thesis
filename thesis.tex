...
Modern data center operators are fighting tooth and claw for efficiency. In prior decades due to
Moors law and Dennard scaling data center operators to simply wait for the next generation of
hardware to arrive for higher performance at lower monetary and energy costs. Both trends have
slowed down dramatically -- CPU clock speeds and memory density, while still improving, have
comparatively stagnated. The result is new technologies to fill the gap, rather than frequency
scaling CPU's are scaling out and have become monstrously parallel, and common performance critical
applications are being offloaded to accelerators like GPU's, TPU's, DPU's, SmartNICs and FPGA's.
Memory is now increasingly tiered with the advent of NVM, persistent memory, and far memory. 

What should a data center server look like? 20 years ago an operator may have argued for the
simplicity of homogeneity, arguing for a carefully picked set of price and performance optimal
components for a few thousand servers, perhaps with some skew for storage servers. Today that same
operator probably could not name every kind of server in their data center. At the time of writing
EC2 has 84 listed instance types~\cite{ec2-offer} for their customers to design their services. The
trend is clear, in search of efficiency data center and server design is increasingly heterogeneous
with servers being designed for specific applications and workloads.
...
less than 10 years ago~\cite{micron-memorywall}. New memory tiers like NVM and persistent memory
have been introduced to help alleviate memory pressure from applications, but the composition of
these memory tiers, and the software to manage them are still undecided.
...
The benefits of pooling are clear when examining a simple bin packing problem. Consider 2 servers
both provisioned with 4GB of DRAM and 3 jobs each of which requires 2.5GB of memory. In a monolithic
design a scheduler can only place one job per machine or risk swapping to disk. In a disaggregated
model the 4GB of memory would be placed in an 8GB pool which could easily subdivided into 3 2.5GB
partitions. In the monolithic case the unused memory is stranded, pooling reclaims stranded memory.
More concretely at datacenter levels practitioners have to provision their servers for the sum of
peak demand, when resources are pooled they can be provisioned for the peak of the sum of demand
which can be significantly lower~\cite{supernic, dsnf}.
...
present the design and implementation of two systems designed for disaggregated memory and with
sharing and contention reduction as a core concern. The first system {\sword} is a middlebox
approach for removing contention from shared data structures. The high level insight behind {\sword}
is to leverage the fact that at rack-scale TORs have a serialized view of traffic to and from
servers. Using this idea {\sword} caches data structure state on a programmable switch 
